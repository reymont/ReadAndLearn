...
output:
  logstash:
    enabled: true
    hosts:
      - logstash.service.ob.local:5001
...
docker tag docker.elastic.co/beats/filebeat:5.4.3 \
  image.service.ob.local:5000/runtime/filebeat:5.4.3
docker build -t image.service.ob.local:5000/runtime/filebeat:5.4.3 .
docker run -d --name fb2 --privileged=true \
  -v /var/lib/docker/containers:/var/lib/docker/containers \
  -v /var/log/nginx:/var/log/nginx \
  image.service.ob.local:5000/runtime/filebeat:5.4.3;docker logs -f fb2
# 保存filebeat
docker save image.service.ob.local:5000/runtime/filebeat:5.4.3 > filebeat.tar
# 拷贝
scp filebeat.tar 192.168.31.224:~/filebeat.tar