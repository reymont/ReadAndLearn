...
busybox, etc) that are in use by containers. A naive（天真，简单） `docker rmi $(docker images
-q)` will leave images stripped(剥离的) of all tags, forcing docker to re-pull the
