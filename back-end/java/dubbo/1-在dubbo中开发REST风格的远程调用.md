在dubbo中开发REST风格的远程调用 - CSDN博客 http://blog.csdn.net/qq_35943347/article/details/72831450?locationNum=8&fps=1
...
不支持用@Context annotation对服务的实例字段注入 ServletConfig、ServletContext、HttpServletRequest、HttpServletResponse等等，但可以支持对服务方法参数的注入。但对某些特定REST server实现，（祥见前面的叙述），也不支持对服务方法参数的注入。
REST常见问题解答（REST FAQ）
